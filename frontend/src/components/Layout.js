...
import { 
  FaTachometerAlt, FaBuilding, FaUsers, FaCreditCard, 
  FaWrench, FaChartLine, FaSignOutAlt, FaFileInvoice, 
  FaEnvelope, FaBlog, FaMapMarkerAlt 
} from 'react-icons/fa';
        ...
        <nav className="mt-6">
          {menuItems.map(item => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `flex items-center gap-3 px-6 py-3 text-gray-300 hover:bg-gray-700/50 transition duration-200 ${
                  isActive ? 'bg-gray-700 text-cyan-400 border-r-2 border-cyan-400' : ''
                }`
              }
            >
          ...
          <button
            onClick={logout}
            className="flex items-center gap-3 px-6 py-3 text-red-400 w-full hover:bg-gray-700/50 mt-4"
          >
